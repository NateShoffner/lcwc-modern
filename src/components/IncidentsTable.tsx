...
function NoIncidentsTable() {
  return <Alert variant="info">There are currently no active incidents.</Alert>;
}
    ...
    <Table striped bordered hover responsive>
      ...
      <tbody>
        {incidents.map((incident, index) => (
          <tr
            className={incident.category.toLowerCase()}
            key={incident.id}
            style={{ cursor: "pointer" }}
            onClick={() => {
              navigate(`/incident/${incident.number}`, {
          ...
          >
            <td>
              {new Intl.DateTimeFormat("en-US", {
                ...
                minute: "2-digit",
                second: "2-digit",
              }).format(Date.parse(incident.meta.added_at))}
            </td>
            <td>{normalizeDescription(incident.description)}</td>
            <td>
              {normalizeName(incident.intersection)}
              <br />
              {normalizeName(incident.municipality)}
            </td>
            <td>
...
function IncidentsTable({ incidents }: IncidentTableProps) {
  return incidents.length > 0 ? (
    <PopulatedIncidentsTable incidents={incidents} />
  ) : (
    <NoIncidentsTable />
  );
}
